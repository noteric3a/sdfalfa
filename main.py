...
async def create_embed(bot_type, targetTime, message, binary, bot_type2, instant_response_time = None):
    ...
    if bot_type2 == 1 or bot_type2 == 2:
        ...
        embede.add_field(name="Current day", value=gg.split("-")[0].strip(), inline=False)
        embede.add_field(name="Sent message | Date and Time", value=gg.split("-")[1].strip(), inline=True)
...
async def gm_bot(interaction: discord.Interaction, hour: int = None, minute: int = None, second: int = None, instant_response: bool = None, auto_hi: bool = None):
    global gm_running
    ...
    while recursive == 200:
        ...
        if clear_time == 0:
            recursive = await gm(interaction, hour, minute, second, instant_response, auto_hi)
            clear_time += 1
        else:
...
async def gm(interaction: discord.Interaction, hour: int = None, minute: int = None,
             second: int = None, instant_response: bool = None, auto_hi: bool = None):
    global gm_embed
    global gm_running
    ...
    if not auto_hi or auto_hi is None:
        pass
    else:
        gm_auto_hi = True
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f'http://{webserver_ip}:5000/gm') as resp:
                        if resp.status == 200:
                            ...
                            # Update the embed with a success message
                            current_days = day(bot_type=1)
                            makeCurrentDay(bot_type=1, days=current_days, message=gm_message,
                                           mcd_target_time=instant_response_gm_target_time)
                            logger("gm_bot", "Recorded the current day!")
...
@app.route('/get_auto_hi', methods=['GET'])
def get_auto_hi():
    global gm_auto_hi
